...
///
- (void)imageFor:(KarmiesCategory * _Nonnull)category reload:(BOOL)reload sync:(BOOL)sync completion:(void (^ _Nullable)(UIImage * _Nullable))completion;
...
/// \param completion called with the image once loaded, synchronously if cached in memory else asynchronously unless sync is specified
///
- (void)imageWithNamed:(NSString * _Nonnull)name size:(enum KarmiesImageSize)size reload:(BOOL)reload sync:(BOOL)sync completion:(void (^ _Nullable)(UIImage * _Nullable))completion;
/// Returns an image for an emoji.
/// \param emoji the emoji
///
/// \param size the size to return (normal or large), defaults to normal
///
/// \param reload flag to reload from network ev xen if the image is cached
///
/// \param sync flag to load synchronously even from disk, such as for during a drawing operation
///
/// \param completion called with the image once loaded, synchronously if cached in memory else asynchronously unless sync is specified
///
- (void)imageFor:(KarmiesEmoji * _Nonnull)emoji size:(enum KarmiesImageSize)size reload:(BOOL)reload sync:(BOOL)sync completion:(void (^ _Nullable)(UIImage * _Nullable))completion;
@end
...
- (UIViewController * _Nullable)presentFeatureController:(UIViewController * _Nonnull)viewController SWIFT_WARN_UNUSED_RESULT;
- (void)dismissFeatureController;
- (nonnull instancetype)initWithNibName:(NSString * _Nullable)nibNameOrNil bundle:(NSBundle * _Nullable)nibBundleOrNil OBJC_DESIGNATED_INITIALIZER;
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)aDecoder OBJC_DESIGNATED_INITIALIZER;
@end
